...
ipak(pkg)
readxl::read_excel("c:/work/data_rg.xlsx",sheet="Sheet2")%>%handling%>%as.data.table%>%mutate(g20_df=CLIG20-lag(CLIG20))
rg <-readxl::read_excel("c:/work/data_rg.xlsx",sheet="Sheet2")%>%handling%>%as.data.table%>%mutate(g20_df=CLIG20-lag(CLIG20))%>%melt(id.vars="STD_DT")%>%na.omit

# 패키지 불러오기
set.seed(33)
data1 <-  rg%>%filter(variable=="g20_df")%>%dcast(STD_DT~variable)%>%filter(STD_DT>="2000-01-01")%>%dplyr::select(g20_df)%>%na.omit%>%t%>%c
data1%>%diff
data2 <-  rg%>%filter(variable=="INFG20")%>%dcast(STD_DT~variable)%>%filter(STD_DT>="2000-01-01")%>%dplyr::select(INFG20)%>%na.omit%>%t%>%c
...
x1 <- rg%>%filter(variable=="g20_df")%>%dcast(STD_DT~variable)%>%filter(STD_DT>="2000-01-01")%>%dplyr::select(STD_DT)
x2 <- rg%>%filter(variable=="g20_df")%>%dcast(STD_DT~variable)%>%filter(STD_DT>="2000-01-01")%>%dplyr::select(STD_DT)
data.frame(STD_DT=x1,data1,STATE1=post_probs1$state)

TMP <- data.frame(STD_DT=x1,data1,STATE1=post_probs1$state)%>%left_join(data.frame(STD_DT=x2,data2,STATE2=post_probs2$state),by="STD_DT")%>%
  mutate(REGIME1=STATE1*(STATE2+1))%>%na.omit
